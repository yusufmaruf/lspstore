    ...
    <div class="row clearfix">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="body">
                    <form action="{{ route('product.update', ['product' => $product->idProduct]) }}" id="basic-form"
                            ...
                            <select class="form-control bs4" name="idCategory" style="width: 100%;">
                                @foreach ($categories as $item)
                                    <option value="{{ $item->idCategory }}"
                                        @if ($item->idCategory == $product->idCategory) selected @endif>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>Image</label>
                            <br>
                            <img src="{{ asset('storage/' . $product->image) }}" height="100px" class="mb-2"
                                width="100px" alt="">
                            <input type="file" name="image" class="form-control">
                        </div>
                        <div class="form-group">
                            <label for="name" class="form-label">Desc</label>
                            <textarea id="editor" name="desc" class="form-control">{{ $product->desc }}</textarea>
                        </div>
